...
parser = argparse.ArgumentParser(description="Chat with an SFT fine-tuned model")
...
if args.hf_model:
    checkpoint_path = hf_hub_download(repo_id=args.hf_model, filename="chatling.pth")
    print(f"Downloaded checkpoint from Hugging Face Hub ({args.hf_model}): {checkpoint_path}")
else:
    checkpoint_path = get_latest_checkpoint('checkpoints/sft', 'model_step*.pth')
